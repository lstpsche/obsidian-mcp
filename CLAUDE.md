...
- **CI/CD workflows:** Three GitHub Actions workflows: `ci.yml` runs `cargo fmt --check` (Ubuntu only) + `cargo clippy` + `cargo test` on a 3-OS matrix (ubuntu-latest, macos-latest, windows-latest). `release.yml` triggers on `v*` tags, builds 5 targets (x86_64-unknown-linux-gnu, aarch64-unknown-linux-gnu, x86_64-apple-darwin, aarch64-apple-darwin, x86_64-pc-windows-msvc), creates a GitHub Release with checksums, and publishes to crates.io. `edge.yml` triggers on push to `main` (and `workflow_dispatch`), builds the same 5 targets, and publishes a rolling `edge` prerelease on GitHub. Edge archives use `obsidian-mcp-edge-{target}` naming (no version number). The `edge` tag is deleted and recreated on each run via `gh` CLI. A concurrency group (`edge-release`) cancels in-progress builds when a newer push arrives. Edge releases never publish to crates.io.
