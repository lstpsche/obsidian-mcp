...
- **`parse_embedding_response` shared function:** A single `#[cfg(feature = "embeddings-api")]` function in `embeddings.rs` parses OpenAI-compatible embedding API JSON into `Vec<Vec<f32>>`. Used by both `embed_batch_api` (production) and unit tests (directly). Avoids duplicating parsing logic.
