...
- **`vault_list` metadata compatibility:** `include_metadata` defaults false so existing list-mode string arrays and tree text remain unchanged. In list mode, `true` enriches indexed case-insensitive Markdown paths from `NoteMetadata` with filename-stem title, tags, size, and optional timestamps; directories, non-Markdown files, excluded/unindexed notes, and list/lookup races stay path-only. Only `NoteNotFound` is downgraded; other errors propagate. Tree mode plus metadata is `INVALID_PARAMS`.
