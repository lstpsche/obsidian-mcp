...
|-----------|-------|---------|
| MCP protocol | `rmcp` 3.2 | Server, tools, stdio + Streamable HTTP transport |
...
- **TCP health test servers:** Consume the entire HTTP request header terminator before replying and closing. A single TCP read can return a partial request; closing with unread bytes can abort the connection on Windows before response bytes reach the client. Bound fixture reads by timeout and size.


- **schemars version:** rmcp 3.2 uses schemars 1.x (not 0.8). Use `schemars = "1.0"` as direct dep or `use rmcp::schemars;` re-export. The `server` feature activates `dep:schemars` automatically.
...
- **notify-debouncer-mini event erasure:** Version 0.7 erases event kinds into `Any`/`AnyContinuous`. Both vault and daemon watchers use `new_change_debouncer` with `ChangeWatcher` to discard read-only access events before debouncing; otherwise Linux file-open events turn indexing reads into an endless reindex loop, including for malformed YAML. Write-close events and watcher errors are preserved. Debounced paths are disambiguated by filesystem state: existing paths are reindexed, missing paths removed. Renames produce old/new paths. `DebounceEventResult` contains a single `notify::Error`, not a vector.
...
- **rmcp `Parameters` import:** The `Parameters` extractor is at `rmcp::handler::server::wrapper::Parameters`, NOT `rmcp::handler::server::tool::Parameters` (the latter is a private re-export path). The MCP quickstart docs reference rmcp 0.3 which had a different module layout.
- **rmcp `ServerInfo` is `#[non_exhaustive]`:** `ServerInfo` is a type alias for `InitializeResult` which is `#[non_exhaustive]`. Cannot use struct literal syntax. Use builder: `ServerInfo::new(capabilities).with_server_info(...).with_instructions(...)`.
- **rmcp `ServiceExt::serve`:** Import `rmcp::ServiceExt` to get the `.serve()` method. For stdio: `.serve(rmcp::transport::io::stdio())`. Returns `RunningService`; call `.waiting().await?` to block until shutdown.
...
- **HTTP transport config:** `legacy_session_mode: true` preserves initialize/session behavior for older clients; MCP 2026-07-28 requests use stateless discovery and execution. `json_response: true` requests JSON where supported; legacy sessions still use SSE. RMCP supplies discovery from `ServerHandler::get_info()` and its supported protocol versions. Every handler shares the same `Vault`.
...
- **RMCP content types:** Tool text uses `ContentBlock::text`; the former `Content` type is not available in RMCP 3.2.
...
- **Tool-layer param validation error codes:** `VaultError::Other(...)` maps to `ErrorCode::INTERNAL_ERROR`, not `INVALID_PARAMS`. For parameter validation in tool handlers (invalid action/query/view/type, missing required param), construct `ErrorData::new(ErrorCode::INVALID_PARAMS, msg, None::<serde_json::Value>)` directly. Only use `VaultError` + `?` for errors originating from vault-layer method calls. RMCP 3.2 converts argument-deserialization failures into tool results with `isError: true`; these must still reject invalid inputs before any vault mutation. Handler-originated validation errors remain JSON-RPC errors.
- **Read profile:** Excludes the entire `frontmatter` tool because it also exposes `set` and `remove`; tool filtering applies to names, not individual actions. Frontmatter remains readable as raw note content via `note_read`.
- **Runtime tool router binding:** `#[tool_handler(router = self.tool_router)]` is required so generated listing, lookup, and dispatch use the filtered instance router. The bare attribute constructs `Self::tool_router()` and bypasses runtime filtering. Test filtering through JSON-RPC requests, including rejected calls, rather than only inspecting router fields.
...
- **HTTP tool filtering (`X-Obsidian-Tools`):** Middleware scopes the parsed disabled set in `REQUEST_DISABLED_TOOLS`; the service factory unions it with server-wide restrictions. Legacy sessions retain the initialization filter; modern stateless requests must carry their own filter. Invalid profile names or header encoding return HTTP 400, and missing task-local context fails handler construction. The middleware applies only to MCP routes, not `/health`.
