...
- **API 429 retry:** `embed_batch_api()` retries once on HTTP 429 (rate limited), respecting the `Retry-After` header with a 30s cap. Only a single retry is attempted — further failures propagate as `VaultError::Embedding`.
