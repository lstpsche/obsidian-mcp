...
- **Embedding readiness and LKG:** Status is `warming`, `ready`, or `degraded`, with queryable/indexed/total/pending fields. A compatible completed cache is pruned to current index membership and may serve as same-space last-known-good during refresh; failures retain the old vector and matching old hash together. A new failed note remains absent. Deleted/excluded paths are filtered again at result construction.
...
- **Embedding runtime shutdown:** The final runtime handle marks the coordinator dead, synchronizes with atomic persistence, wakes and aborts the async worker, and invalidates pending generations. A started `spawn_blocking` inference cannot be cancelled by Tokio, but its result cannot mutate the published store or cache after runtime drop returns.
...
- **Embedding cache relocation:** Local cache location remains `{mcp_data}/embeddings/embeddings.bin`; the legacy `{vault}/.obsidian/obsidian-mcp/embeddings.bin` is best-effort copied when the destination is absent and is never deleted. The current runtime rejects the legacy payload contract and rebuilds it in the background. All persistence belongs to the runtime coordinator; watchers only submit intents. The daemon independently uses its context-owned cache path.
