...
- **Legacy embedding cache migration path:** Cache relocation belongs to the managed embedding coordinator, never MCP/Vault startup. For a daemon context it prefers the active local `.obsidian-mcp/embeddings/embeddings.bin`, then the older `.obsidian/obsidian-mcp/embeddings.bin`. Relocation uses a same-directory temporary file plus atomic no-clobber publication, is idempotent, and never deletes the source; strict identity validation still decides whether the relocated payload can be served.
